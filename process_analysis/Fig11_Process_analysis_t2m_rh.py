...
# hourly boxplot
tempdiff=(dsirr_newtime.TEMP2.where(dsirr_AMJ['IRRIFRAC']>irrilimit))[:,0,:,:]\
    - (dsnoirr_newtime.TEMP2.where(dsnoirr_AMJ['IRRIFRAC']>irrilimit))[:,0,:,:]
tempdiffhour=tempdiff.groupby('time.hour').mean('time', skipna=True).stack(space=['rlon','rlat']).reset_index('space').drop(['rlon','rlat','lon','lat'])
...
rhhourdiff=((rh_irri_series_AMJ.where(mdsirri_extended['IRRIFRAC']>irrilimit))\
                .mean(['rlat','rlon'], skipna=True).groupby("time.hour").mean(skipna=True))\
...
rhdiff=(rh_irri_series_AMJ.where(mdsirri_extended['IRRIFRAC']>irrilimit))\
               - (rh_noirri_series_AMJ.where(mdsnoirri_extended['IRRIFRAC']>irrilimit))
rhdiffhour=rhdiff.groupby('time.hour').mean('time', skipna=True).stack(space=['rlon','rlat']).reset_index('space').drop(['rlon','rlat','lon','lat'])
...
rh_irri_months=(rh_irri_months.where(mdsirri_extended['IRRIFRAC']>irrilimit)).mean(['rlat','rlon'], skipna=True)
rh_noirri_months=(calculate_rh(tdew_noirri_months,t_noirri_months))
rh_noirri_months=(rh_noirri_months.where(mdsirri_extended['IRRIFRAC']>irrilimit)).mean(['rlat','rlon'], skipna=True)
...
ticks=[-2, -1, 0, 1,  2]
rotplot = plot_rotvar(fig, tempspatialdiff, ax1, cax1, label='T2Mean [ΔK]', unit='[ΔK]',
    cmap='RdBu_r', levels=levels, extend_scale='both', ticks=ticks, cbar_orient='horizontal')
ax1.set_title(' ')

ax2 = fig.add_subplot(2, 3, 2)
box=df_tempdiffhour.boxplot(column='TEMP2', by='hour',  ax=ax2, whis=[5,95])#, showfliers=False)       
...
irrilimit=0.0
for var in varlist: 
    tempmonthdiff=calculate_monthmeans_diff(var,irrilimit, mdsirri_extended, mdsnoirri_extended)[:,0]
    tempmonthdiff.plot.line(ax=ax3, marker='.')
    ax3.set_xticks(ticks=tempmonthdiff.time.values, labels=tempmonthdiff.time.dt.strftime('%b').values)
...
rotplot = plot_rotvar(fig, rhdiff_AMJ, ax4, cax4, unit='[Δ%]', label='relative humidity [Δ%]', cmap='RdBu_r',\
                       extend_scale='both', levels=levels4, ticks=ticks,  cbar_orient='horizontal')
ax4.set_title(' ')


ax5 = fig.add_subplot(2, 3, 5)
box=df_rhdiffhour.boxplot(column='RH2', by='hour',  ax=ax5, whis=[5,95])#, showfliers=False)       
...
ax6.set_xticks(rhdiff_months.time.values, labels=rhdiff_months.time.dt.strftime('%b').values)

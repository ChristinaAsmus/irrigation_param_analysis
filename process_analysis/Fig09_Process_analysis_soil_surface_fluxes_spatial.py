...
rotplot = plot_rotvar(fig, lhfl_diff, ax1, cax1, label='latent heat flux [ΔWm$^{-2}$]',\
    unit='[ΔWm$^{-2}$]', cmap='RdBu_r',levels=levels, extend_scale='both', ticks=ticks, cbar_orient='horizontal')

ax2 = fig.add_subplot(1, 3, 2, projection=rotated_pole)
cax2 = fig.add_axes([0.42, 0.01, 0.19, 0.04])

rotplot = plot_rotvar(fig, shfl_diff, ax2,  cax2,  label='sensible heat flux [ΔWm$^{-2}$]',\
    unit='[ΔWm$^{-2}$]',cmap='RdBu_r', levels=levels, extend_scale='both', ticks=ticks,  cbar_orient='horizontal')
    

ax3 = fig.add_subplot(1, 3, 3, projection=rotated_pole)
cax3 = fig.add_axes([0.69, 0.01, 0.19, 0.04])
levels3=[-1. , -0.8, -0.6, -0.4, -0.2,  0.2,  0.4,  0.6,  0.8,  1.]

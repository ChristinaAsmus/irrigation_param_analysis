...
varlist=['TEMP2','T2MAX','T2MIN', 'WSECHIRR', 'WSMXIRR', 'APRL', 'APRC', 'EVAPIRR','EVAP', 'EBSOIL', 'ETRANS','ALAI_PFI', 'ESKIN']
var_num_list=['167','201','202', '701', '728','142','143', '739','182', '013','012', '746','302']
month=8
...
for i in range(len(rlat_list)):
     temp2_noirr=(dsnoirr_newtime.isel(rlat=slice(rlat_list[i][0],rlat_list[i][1]),rlon=slice(rlon_list[i][0],rlon_list[i][1]))\
     ['TEMP2'][:,0,:,:]).resample(time='D').mean(skipna=True).mean(dim=['rlon','rlat'], skipna=True)-273.15
     temp2_irr=(dsirr_newtime.isel(rlat=slice(rlat_list[i][0],rlat_list[i][1]),rlon=slice(rlon_list[i][0],rlon_list[i][1]))\
     ['TEMP2'][:,0,:,:]).resample(time='D').mean(skipna=True).mean(dim=['rlon','rlat'], skipna=True)-273.15
    # temp2_series=temp2.to_series() 
     t2max_noirr=(dsnoirr_newtime.isel(rlat=slice(rlat_list[i][0],rlat_list[i][1]),rlon=slice(rlon_list[i][0],rlon_list[i][1]))\
     ['T2MAX'][:,0,:,:]).resample(time='D').max(skipna=True).mean(dim=['rlon','rlat'], skipna=True)-273.15
     t2max_irr=(dsirr_newtime.isel(rlat=slice(rlat_list[i][0],rlat_list[i][1]),rlon=slice(rlon_list[i][0],rlon_list[i][1]))\
     ['T2MAX'][:,0,:,:]).resample(time='D').max(skipna=True).mean(dim=['rlon','rlat'], skipna=True)-273.15
     #t2max_series=t2max.to_series() 
     t2min_noirr=(dsnoirr_newtime.isel(rlat=slice(rlat_list[i][0],rlat_list[i][1]),rlon=slice(rlon_list[i][0],rlon_list[i][1]))\
     ['T2MIN'][:,0,:,:]).resample(time='D').min(skipna=True).mean(dim=['rlon','rlat'], skipna=True)-273.15  
     t2min_irr=(dsirr_newtime.isel(rlat=slice(rlat_list[i][0],rlat_list[i][1]),rlon=slice(rlon_list[i][0],rlon_list[i][1]))\
     ['T2MIN'][:,0,:,:]).resample(time='D').min(skipna=True).mean(dim=['rlon','rlat'], skipna=True)-273.15  
     ...
     temp2_noirr.plot.line(ax=ax1, color='blue', linestyle='--', label='T2Mean not irrigated')
     t2max_irr.plot.line(ax=ax1, color='orange', linestyle='-',label='T2Max irrigated')
     t2max_noirr.plot.line(ax=ax1, color='orange',linestyle='--', label='T2Max not irrigated')
     t2min_irr.plot.line(ax=ax1, color='green', linestyle='-', label='T2Min irrigated')
     t2min_noirr.plot.line(ax=ax1, color='green', linestyle='--',label='T2Min not irrigated')
     ax1.set_ylim(20,38)
...
for i in range(len(rlat_list)):
     ...
     ws_noirr=(relws_noirr.isel(rlat=slice(rlat_list[i][0],rlat_list[i][1]),rlon=slice(rlon_list[i][0],rlon_list[i][1])))\
     .resample(time='D').mean(skipna=True).mean(dim=['rlon','rlat'], skipna=True)
    # ws_noirr_series=ws_noirr.to_series()  
     
     relws_irr=dsirr_newtime['WSECHIRR']/dsirr_newtime['WSMXIRR']
     ws_irr=(relws_irr.isel(rlat=slice(rlat_list[i][0],rlat_list[i][1]),rlon=slice(rlon_list[i][0],rlon_list[i][1])))\
     .resample(time='D').mean(skipna=True).mean(dim=['rlon','rlat'], skipna=True)
...
ws_noirr.plot.line(ax=ax2, color='blue',linestyle='--',  label='soil moisture not irrigated')
ax2.set_ylim(0,1)
...
def calculate_sum_precip(data, var1, var2, i):
    precip=(data.isel(rlat=slice(rlat_list[i][0],rlat_list[i][1]),rlon=slice(rlon_list[i][0],rlon_list[i][1]))\
            [var1])+(data.isel(rlat=slice(rlat_list[i][0],rlat_list[i][1]),rlon=slice(rlon_list[i][0],rlon_list[i][1]))\
            [var2])
    precip_mean=(precip.resample(time='D').sum().mean(dim=['rlon','rlat'], skipna=True))
    return precip_mean


precip_noirr_series=calculate_sum_precip(dsnoirr_newtime,'APRL','APRC',i).to_series()
...
df_precip[['precip_irr','precip_noirr']].plot.bar(ax=ax3, legend=True, width=0.6, zorder=1)
...
colors={'evapotranspiration':'black', 'evaporation of soil':'brown', 'evaporation of skin reservoir':'blue',\
...
def calculate_daily_sum_evapos(var, i, irrilimit):
    dsirr_month=((dsirr_newtime.isel(rlat=slice(rlat_list[i][0],rlat_list[i][1]),\
             rlon=slice(rlon_list[i][0],rlon_list[i][1]))[var])\
             .groupby('time.day').sum()).mean(dim=['rlon','rlat'], skipna=True)
    
    dsnoirr_month=((dsnoirr_newtime.isel(rlat=slice(rlat_list[i][0],rlat_list[i][1]),\
             rlon=slice(rlon_list[i][0],rlon_list[i][1]))[var])\
             .groupby('time.day').sum()).mean(dim=['rlon','rlat'], skipna=True)
    diff_month=dsirr_month-dsnoirr_month
    return  dsirr_month, dsnoirr_month, diff_month

datelist=pd.to_datetime(['2017-08-01','2017-08-02','2017-08-03','2017-08-04','2017-08-05','2017-08-06','2017-08-07'],\
                       format='%Y-%m-%d')
...
for i in range(len(rlat_list)):
    ...
    data = {'date': np.concatenate((irri_df.index, noirri_df.index, \
                                 irri_df.index, noirri_df.index, \
                                 irri_df.index, noirri_df.index, \
                                 irri_df.index, noirri_df.index)),
            '[mm]': np.concatenate((a_1, a_2, b_1, b_2, c_1, c_2, d_1, d_2)),
            'variable': ['evapotranspiration']*len(a_1) + ['evapotranspiration']*len(a_2) + \
                        ['evaporation of soil']*len(b_1) + ['evaporation of soil']*len(b_2) +\
                        ['transpiration of vegetation']*len(c_1) + ['transpiration of vegetation']*len(c_2) +\
                        ['evaporation of skin reservoir']*len(d_1) + ['evaporation of skin reservoir']*len(d_2),\
            'simulation': ['irrigated']*len(a_1) + ['not irrigated']*len(a_2) + ['irrigated']*len(b_1) + ['not irrigated']*len(b_2)+\
                          ['irrigated']*len(c_1) + ['not irrigated']*len(c_2) + ['irrigated']*len(d_1) + ['not irrigated']*len(d_2)}
    
    ax4 = fig.add_subplot(4,1,4)
    plt.rcParams.update(params)
    p = sns.lineplot(data=data, x='date', y='[mm]', hue='variable', style='simulation', palette=colors, ax=ax4)
    p.legend_.remove()

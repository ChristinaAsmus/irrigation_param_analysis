...
from analysis_functions.functions_calculations import calculate_sum_precip
...
dsnoirr_newtime = dsnoirr_newtime.sel(
    time=dsnoirr_newtime.time.dt.month.isin([4, 5, 6, 7, 8])
)
...
for i in range(len(rlat_list)):
    ...
    precip_series = calculate_sum_precip(
        "APRL", "APRC", dsnoirr_newtime, rlat_list, rlon_list, i
    ).to_series()
    ...
    df_combined.precip.plot.bar(
        ax=ax1, secondary_y=True, color="royalblue", width=0.6, zorder=-1
    )
    ...
    ax1.right_ax.set_yticklabels(
        labels=np.linspace(0, 20, 9), color="royalblue", fontsize=16
    )
...
fig.subplots_adjust(wspace=wspace)
plt.savefig(str(dir_out) + "/Fig07.png", dpi=300, bbox_inches="tight")
plt.show()

...
def read_efiles(var, var_num, exp_number, year, month): 
    dir_path = '/work/ch0636/g300099/SIMULATIONS/GAR11/remo_results/'+str(exp_number)+'/2017/'
    if month >= 1 and month <= 12: 
        efiles = 'var_series/'+str(var)+'/e'+str(exp_number)+'e_c'+str(var_num)+'_20170'+str(month)+'.nc'
    else: 
        efiles = 'var_series/'+str(var)+'/e'+str(exp_number)+'e_c'+str(var_num)+'_2017*.nc'
...
def read_tfiles(exp_number, year, month): 
    ...
    if month >= 1 and month < 10: 
        tfiles = 'xt/e'+str(exp_number)+'t'+str(year)+'0'+str(month)+'*.nc'#

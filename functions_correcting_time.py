...
def correct_timedim(data):
    ...
    timelist=[str(item)for item in [int(item) for item in (round (num) for num in [t %1*24 for t in data.time.values])]]
...
def correct_timedim_mfiles(data):
    ...
    dsnew=data.assign_coords({"time":time_corr})#.swap_dims({'dim_0':'time'}).drop_vars('dim_0')
    return dsnew